...
export default function TopBar() {
  ...
  // 迁移补齐：词元总量改读 /api/meter（原 hudToken 依赖 state.token_usage，而后端快照
  // 从不含该字段 → 恒不显示）。服务端 meter 为本回合累计（advance 后 flush+清零，
  // 与 Tk 顶栏 `ai_client.token_usage` 口径一致）。
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const m = await getApiClient().meter();
        const t = Number(m.total?.prompt ?? 0) + Number(m.total?.completion ?? 0);
        if (alive) setMeterTokens(t);
      } catch {
        /* 计量不可用不影响顶栏 */
      }
    })();
  ...
  const tokenShown = meterTokens ?? token;
      ...
      <div className="pointer-events-auto flex items-center gap-2 rounded-[3px] border border-gold bg-card px-2.5 py-2 shadow-paper">
        ...
        {tokenShown !== null && (
          <button
            ...
            title="Token 用量明细（点击查看/清零）"
            className="whitespace-nowrap px-1 text-[12px] font-bold text-dim transition hover:text-red"
          >
            词元 ▸ {Number(tokenShown).toLocaleString("en-US")}
          </button>
        )}
