...
def test_execution_rate_direct_no_wolf_penalty():
    """审查 2026-09：狼来了机制已取消——御笔直发恒有 +0.10 加成，不随 wolf_count 变化受罚。"""
    s = _new_state()
    stances = {fn: 1 for fn in s.factions}  # 全员赞成 → net_support 最大
    s.wolf_count = 0
    a = s.calc_decree_execution_rate(stances, is_direct=True)
    s.wolf_count = 3
    b = s.calc_decree_execution_rate(stances, is_direct=True)
    s.wolf_count = 9
    c = s.calc_decree_execution_rate(stances, is_direct=True)
    # 不再有"狼来了"反噬：无论 wolf_count 多少，御笔直发执行率相同（且都为加成态）
    assert abs(a - b) < 1e-9 and abs(b - c) < 1e-9
    # 御笔直发仍高于"非直发"（+0.10 加成保留）
    nd = s.calc_decree_execution_rate(stances, is_direct=False)
    assert a > nd
