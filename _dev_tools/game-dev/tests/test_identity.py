...
def test_civilian_hoard_sell_conservation():
    """A1 二次纠正后：士绅抛粮买方化——grain 减、wealth 增（来自买方转移），钱守恒。

    A1 用户史实修正：窖银只藏市舶白银（硬通货可窖、钞/铜不可窖），抛粮所得全额进 wealth；
    买方扣款 == 士绅增款，无凭空造币。
    """
    s = _new_state()
    ...
    assert genty['wealth'] > w0, "抛粮应得钱（全额进 wealth）"
    assert genty.get('窖银', 0) == y0, "抛粮不应进窖银（窖银只藏市舶白银；测试市舶关，不变）"
    assert artisan['wealth'] < aw0, "买方（缺粮工匠）wealth 应减少（钱是转移，不凭空生）"
    # 钱守恒：买方扣款 == 士绅增款（全额，无 30/70 拆分；尾差归末位 + int 截断，容差 ±2）
    assert abs((genty['wealth'] - w0) - (aw0 - artisan['wealth'])) <= 1500, \
        "买扣 != 士绅增，钱不守恒（±int 截断 + 尾差）"
