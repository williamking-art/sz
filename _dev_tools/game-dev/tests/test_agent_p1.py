...
def test_diplomacy_contract_applies():
    ...
    审查注（flaky 修复）：固定随机种子。本路径上 settlement_steps._settle_military_diplomacy
    对西夏 attitude 另有 10% 概率随机游走（±10），叠加契约 ±8 会让本断言随机失败
    （实测 3 次运行出现 d=0 / d=17）。seed(0) 的首个 random.random()≈0.844>0.1，
    使该分支确定不触发，断言只度量契约本身。
    """
    import random
    random.seed(0)
...
def test_military_contract_applies():
    """军事契约：训练/士气 ±2~±6；AI 不得直接增兵（增募走批红军令）；征发 cost 守恒扣款。"""
    from core.settlement_steps import _settle_military_diplomacy
    s = _new_state()
    ...
    # 审查 P0：AI 军事契约不得直接 +troops（防铸兵）；可减员
    assert u0.troops <= troops0
    assert s.treasury == t0 - 350000   # levy 中 = 35万 守恒扣款


def test_relief_contract_applies():
    """灾荒契约：赈济档位换算 + 太仓守恒扣。"""
    from core.settlement_steps import _settle_disaster
    s = _new_state()
...
def test_conservation_agent_does_not_touch():
    """守恒铁律 + 按需唤醒：economy 必调（核心推演）；diplomacy 因 external 常醒；
    无军情/无灾荒时 military/relief 不唤醒（不违反守恒）；有军情/灾荒时唤醒。"""
    ...
    class _AgentFakeAI(FakeAIClient):
        ...
        def economy_decide(self, posture, state=None):
            self.contract_calls.append("economy")
    ...
    # 场景 1：无军情/无灾荒 → economy+diplomacy 唤醒，military/relief 不唤醒
    before = _snap(s)
    fake = _AgentFakeAI()
    settle_turn(s, ai_client=fake)
    assert "economy" in fake.contract_calls, "economy 为核心推演，必须唤醒"
    assert "diplomacy" in fake.contract_calls, "diplomacy 因 external 常醒"
    assert "military" not in fake.contract_calls, "无军情时 military 不应唤醒（省 token）"
    assert "relief" not in fake.contract_calls, "无灾荒时 relief 不应唤醒（省 token）"
    # 守恒：settle 前后 money 差分无凭空跳变（agent 只给档位词，守恒步程序介入）
    after = _snap(s)
    assert abs(after - before) < 50_000_000

    # 场景 2：真实军情（士气低落 <30）→ military 唤醒
    s2 = _new_state()
    u = max(s2.army_units, key=lambda x: x.troops)
    u.morale = 10
    fake2 = _AgentFakeAI()
    settle_turn(s2, ai_client=fake2)
    assert "military" in fake2.contract_calls, "有军情（士气低落）时 military 应唤醒"
    assert "economy" in fake2.contract_calls

    # 场景 3：真实灾荒（disaster_severity>0）→ relief 唤醒
    s3 = _new_state()
    s3.disaster_severity = 2
    fake3 = _AgentFakeAI()
    settle_turn(s3, ai_client=fake3)
    assert "relief" in fake3.contract_calls, "有灾荒（severity>0）时 relief 应唤醒"
    assert "economy" in fake3.contract_calls
