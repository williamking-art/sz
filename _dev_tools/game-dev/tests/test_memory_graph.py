...
def test_save_load_and_archive():
    """存档往返（SQLite .db）+ 归档（低权重旧史 archived 标记，不物理删除）。"""
    g = MemoryGraph()
    g.turn = 20
    g.add_entity("a", "minister", "甲", {}, turn=0)
    ...
    assert g.save(3)
    assert os.path.exists(_db_path(3)), "记忆库应落盘为 slot_3.db"
    g2 = MemoryGraph()
    assert g2.load(3)
    assert g2.turn == 20
    moved = g2.archive(3)  # w_eff 0.1×e^-0.6 < 0.25 → 归档
    assert moved >= 1
    # 归档 = db 内 archived 标记，不物理删除、不再写 archive JSON
    assert any(r.get("archived") for r in g2.relations), "归档关系应打 archived 标记"
    assert not os.path.exists(_mem_path(3, archive=True)), "T7 起不再写 archive JSON"
    if os.path.exists(_db_path(3)):
        os.remove(_db_path(3))
    if os.path.exists(_mem_path(3)):
        os.remove(_mem_path(3))


def test_corrupt_rebuild():
    """损坏（旧 JSON 迁移失败 / db 损坏）→ load 重建空图（不阻断游戏）。"""
    # 用安全槽位 78：真实存档目录可能有 slot_4.db 残留，避免撞真实存档
    os.makedirs(SAVE_DIR, exist_ok=True)
    for p in (_db_path(78), _mem_path(78)):
        if os.path.exists(p):
            os.remove(p)
    with open(_mem_path(78), "w", encoding="utf-8") as f:
        f.write("{ 损坏 json !!!")
    g = MemoryGraph()
    ok = g.load(78)
    assert ok is False
    assert g.entities == {} and g.relations == []
    # 损坏 JSON 未被删除（保留，供人工诊断），也不产生半写 db
    assert os.path.exists(_mem_path(78))
    if os.path.exists(_db_path(78)):
        os.remove(_db_path(78))
    if os.path.exists(_mem_path(78)):
        os.remove(_mem_path(78))
