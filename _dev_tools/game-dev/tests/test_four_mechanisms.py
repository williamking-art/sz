...
def test_estate_constants():
    ...
    assert set(INVEST_BASE) == {"农业", "水利", "工坊", "商铺", "漕运", "军器", "科技"}  # +科技领域（对齐）
...
def test_estate_growth_jingkang_anchor():
    ...
    守恒（审查 P0）：增长来源 = 官僚 POP wealth 池，测试每月补俸模拟薪俸入池。"""
    ...
    # 模拟 25 年（300 月）膨胀；每月向官僚 wealth 注入薪俸（与结算俸给同量级），
    # 保证家产膨胀有守恒来源，而非凭空铸币。
    for _ in range(300):
        for p in s.prefectures.values():
            guan = p["pops"]["官僚"]
            guan["wealth"] = guan.get("wealth", 0) + 2_000_000
