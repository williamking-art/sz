...
- **代码质量重检**：对批 0–7 新增代码静态审查，修复 6 项（Q1 死代码/Q2 未用变量/Q3 未用 import/Q4 未用参数/Q5 `__len__()` 反模式/Q6 `__all__` 缺导出）。修复后全量 **994 passed** + tsc 干净。守恒/拒绝式/旧档兼容/POP 挂载律/金属≠钱/计价位均审查通过。
- **众生相改进（P0+P1）**：P0 scenes 分幕渲染（`_monthly_report_text` 返回 dict+scenes → API → `AdvancePanel` 分幕卡片）；P1 `pop_sentiment_brief` 六民生齿真值摘要注入 monthly_report / event_narrative / civilian_situation prompt（脱敏定性：疲敝/偏低/尚安/安和）。**994 passed** + tsc 干净。
- **众生相改进（③+④）**：③ `_validate_scenes` 分幕约束后验（幕数钳位 2–12/1–8/1–5、字数钳位 20–120、脱敏阿拉伯数字→「数」、空幕占位），接入 monthly/event/civilian 三处；④ `civilian_reaction` prompt 改 3 幕（农人/士绅/商贾）→ `rich_civilian_scenes` 全链路（state/API/Dock/AdvancePanel）→ 民间分幕卡片。**1010 passed** + tsc 干净。
- **众生相改进（⑤）**：`_chapter_scenes` 八章分幕小剧场（程序真值模板：长期局势/密令动向/人物历练/军事/邦交 5 章各 1–2 幕），`GazettePanel` 章内渲染（`scenes` 字段）。**1010 passed** + tsc 干净。**众生相 ①–⑤ 全部收口。**
