...
## 十三、第六轮：AI 架构升级（2026-08-20 晚）

### 改造（pytest 224 全绿，清 PYTHONPATH/CODEBUDDY 环境噪音后）
- **AI 只 Function Call 结构化**：3 工具（update_state/query_state/trigger_event）+ tool_choice required + parse_tool_calls（T1 谷承构）
- **应用层管道**：engine/state_applier.py（验证/合并/守恒校验/cascade/写库/返回叙事层）+ narrative_guard 护栏
- **守恒校验**：钱/粮分组 ΣΔ==0 + reason 补记账 + 硬拒绝（不凭空造灭；修复 wildcard 死路径/set-mul-remove/CLAMP_01 bug）
- **Agent 路由按需唤醒**：core/agent_router.py（military/relief 真实军情/灾荒唤醒，economy 必调，未唤醒不耗 token）
- **契约统一**：ai/contract_adapter.py（34 契约 → {changes, narrative} 双通道）+ 拟旨人 persona 差异 + 句式库迁移运行资源
- **记忆库数据库化**：memory/memory_graph.py SQLite 一轮一库（去重/6回合压缩/12回合总结/精确调动；60 回合验证 summary_10+period_summary_5）
- **异步化**：core/async_ai.py（后台 AI 调用 + 主线程 after 回调）+ 6+1 处 UI 接线 + 结算拆分（UI 不卡）
- **回合时序**：AI 推演 → 系统结算 → 形成叙事
- **技能加载**：7 个 skill（songzuo-dev/agnes-image/web-search-exa/web-access/multi-search-engine 等）

### 验证
- pytest 224 全绿（清环境后）；60 月回放国库 -307万、物价 0.814、农存粮 8.0、记忆 141 实体+31 关系、性能 0.2s
- 新模块已集成（commands.py 引用 agent_router/async_ai 等）

### 待后续
- QA 人工 GUI 验证（AI 调用期间 UI 不卡/加载环/按钮禁用/月折后补）
- exe 重新打包（含新模块）
- self.self.messagebox 笔误清理
- 环境：CodeBuddy sitecustomize shim 干扰测试（需清 PYTHONPATH/CODEBUDDY_* 变量）
