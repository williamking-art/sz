...
【接线状态（审查 D14 考证）】本模块**当前无生产调用方，且这不构成缺陷**——
契约落地按设计仍由各契约自身的结算槽位消费（_economy_ai 等 → 12 步结算），
adapter 是**只读表达层**（本文件第 6-8 行自述「不改任何契约 validate/返回值，
旧消费方零破坏」），故不接线不产生任何行为差异，亦无玩家可见症状；README 已如实
标注「待接线」，无失真声明（区别于 async_ai 的 B7 情形）。
保留价值：34 契约的字段映射已实勘成表（CONTRACT_FIELD_MAP）且有 T5 测试覆盖，
日后做叙事组装器 / 契约审计视图可直接取用。**届时接线不得改为经 adapter 落地状态**
——落地语义保留在契约自身（设计铁律）。
"""
