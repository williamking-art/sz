...
def _ext_pop(regime_type: str, population_wan: int) -> dict[str, dict[str, int]]:
    ...
    total = max(0, int(population_wan)) * 10000
    out = {}
...
CLERK_ENTITLEMENT_GROWTH = 0.0006  # 编制**自我膨胀**（/月）：世袭吏职是私产、豪强请托添额，
#                                「吏额有增无损」——这是"冗吏"真正的来源（不随政务量下降而减）。
#                                0.06%/月 ≈ 0.72%/年；20 年累积 ≈ ×1.15，在政务量只增 7% 的
#                                局里正好产出可观但不夸张的冗吏率（实测 240 月约 7%）。
