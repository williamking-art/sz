...
**设计定稿（三份；货币与财力已实现，官制待实现）**

| 设计 | 文档 | 一句话 | 状态 |
|---|---|---|---|
| 货币口径 | [货币口径规范_M0M1M2.md](../_dev_tools/game-docs/docs/货币口径规范_M0M1M2.md) | 引入 M0/M1/M2/M3 与月度对账恒等式，使"凭空造币/销毁"在构造上可观测；含仓鼠症与国库封桩分层 | ✅ **B-1/B-2/B-3 已生效**：60 月残差 **+43,380,547 → −7,848 贯** |
| 财力消耗 | [财力消耗设计.md](../_dev_tools/game-docs/docs/财力消耗设计.md) | 六层 money sink；核心为 **L1 资产维持费**（随资产存量增长）与 **L2a 军俸补足至史实** | ✅ **L1 已落地**（结算步 Step 3.9）；**L2a 已落地**（军俸 1,349 万贯/年 ＋ 按实付/欠饷） |
| 官制与三冗 | [宋代官制与三冗设计.md](../_dev_tools/game-docs/docs/宋代官制与三冗设计.md) | 官/职/差遣分离、冗官量化定义、政务量 W 界定、地方三层可见性与路级四司、科举离散科次、吏员制度、公务员化 | ⏳ **待实现**（阶段 C） |

**落地顺序（用户定稿）**：① 先修审查问题 → ② 再做 M0/M1/M2/M3 货币口径（含军俸提至史实、money sink）→ ③ 最后做官制完善。
进度：**① ✅ 已完成**（P0×9 ＋ 测试整理）｜**② ✅ 已收口**（B-1 观测 → B-2 守恒 → B-2′ 军俸/欠饷 → B-3 L1 维持费）｜**③ ⏳ 待开始**。详见 [fix_plan_2026-09-18.md](../_dev_tools/game-docs/analysis/fix_plan_2026-09-18.md)。
...
4. **禁止**任何与 POP 平行的独立存量账本（现行 `officials` 与官僚 POP 双账即教训）；
5. 每条改动都要配**守恒断言**并进回归测试。

> **现行反例（待修）**：`prefectures[].officials`（`户数 × 0.00135` 派生）与 `pops["官僚"].size`（`(官+吏) × 9`）是两本账——30 年实跑官僚 POP **+23.1%** 而 `officials` **+0.0%**，国库俸禄支出 30 年**零增长**。详见 [宋代官制与三冗设计.md §13.1](../_dev_tools/game-docs/docs/宋代官制与三冗设计.md)。
