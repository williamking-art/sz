...
from content.data import EVAL_WEIGHTS, EVAL_OUTCOMES
...
def evaluate_game(state) -> dict:
    ...
    # 百姓口碑
    koubei = state.population_satisfaction * 0.7 + (100 - state.refugee_count / max(state.population, 1) * 10000) * 0.3
...
def check_emperor_death(state) -> tuple:
    ...
    if state.emperor_health <= 0:
        return (True, "龙驭上宾")
    # 强制死亡年份 (史实：靖康包围后)
    if state.year >= 1135 and state.emperor_health < 20:
