# -*- coding: utf-8 -*-
"""宋祚 · 军队实体层（core/army_models.py，原 ui/panels_military 下沉）
...
class ArmyUnit:
    ...
    # ---- 欠饷（2026-09-18 阶段 B-3）----
    # 单位：贯（该军**累计未发**军饷）。国库不足时 `_settle_finance` 按各军本月应发
    # 军饷比例分摊短付额并累加于此；语义与 POP 的「欠税」科目对称（见 settlement_steps）。
    # 历史依据：宋代军费占财政大半，"欠饷"是常态问题（本项即其数字化表达）。
    # 默认 0 → 旧档缺字段可安全加载。
    arrears: int = 0
...
def reorganize_xiang_to_jin(state, road: str, ratio: float = 1.0) -> dict:
    ...
    for b, n in list(xu.branches.items()):
        ...
        if xu.branches[b] <= 0:
            del xu.branches[b]
    # 审查 P2-28 修复：原 ratio<1 时保留残编却恒报 dissolved:1（docstring 称「剩余清零移除」），
    # 账目含糊。现语义与返回值一致：整编后无残部 → 该支厢军解散移除（dissolved=1）；
    # 有残部 → 保留（dissolved=0），msg 明示残部人数。
    dissolved = 0
    if not xu.branches:
        dissolved = 1
        try:
            state.army_units.remove(xu)
        except ValueError:
            pass
        _msg = f"[整编] {road} 厢军整编入禁军 {moved_total} 人，该支解散（ratio={ratio:.2f}）"
    else:
        _left = sum(xu.branches.values())
        _msg = (f"[整编] {road} 厢军整编入禁军 {moved_total} 人，"
                f"残部 {_left} 人保留（ratio={ratio:.2f}）")
    return {"ok": True, "moved": moved_total, "dissolved": dissolved, "msg": _msg}
