...
interface StoreShape {
  ...
  /** UI 即时回执（迁移补齐：原 Tk `_log_lines` 运行时内存缓冲；不落档，仅本局，朝报面板消费）。 */
  uiLogs: string[];
}
...
function reducer(s: StoreShape, a: Action): StoreShape {
  switch (a.type) {
    ...
    case "PUSH_UI_LOG":
      // 迁移补齐：UI 即时回执（原 Tk `_log_lines`，最近 200 条，不落档）
      return { ...s, uiLogs: [...s.uiLogs.slice(-199), a.line] };
    case "SET_IN_GAME":
